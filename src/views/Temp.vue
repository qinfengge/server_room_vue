<template>
  <div style="padding: 15px">
    ...
    >
      ...
      <el-table-column prop="wenData" label="温度/C"> </el-table-column>
  ...
  methods: {
    ...
    showEchars(did){
      ...
      this.$nextTick(() =>{
        ...
        request.get("/temp/showEchars/"+did).then(res =>{
          console.log(res)
          this.option.xAxis.data=[]
          this.option.series[0].data=[]
          res.data.forEach(item =>{
            this.option.xAxis.data.push( item.creatTime)
            this.option.series[0].data.push(item.wenData)
          })
    ...
    Delete(id){
      request.delete("/temp/delete/"+id).then(res =>{
        if (res.code==0){
    ...
    deleteBatch(){
      ...
      request.post("/temp/deleteBatch",this.ids).then(res =>{
        if (res.code==0){
          ElMessage.success('删除成功!')
          this.load()
        }
    ...
    updateDevices(){
      //修改
      request.put("/temp/update",this.form).then(res =>{
        if (res.code==0){
