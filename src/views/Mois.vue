<template>
  <div style="padding: 15px">
    ...
    >
      ...
      <el-table-column prop="shiData" label="温度/C"> </el-table-column>
  ...
  methods: {
    ...
    showEchars(did){
      ...
      this.$nextTick(() =>{
        ...
        request.get("/mois/showEchars/"+did).then(res =>{
          console.log(res)
          this.option.xAxis.data=[]
          this.option.series[0].data=[]
          res.data.forEach(item =>{
            this.option.xAxis.data.push( item.creatTime)
            this.option.series[0].data.push(item.shiData)
          })
    ...
    Delete(id){
      request.delete("/mois/delete/"+id).then(res =>{
        if (res.code==0){
    ...
    deleteBatch(){
      ...
      request.post("/mois/deleteBatch",this.ids).then(res =>{
        if (res.code==0){
          ElMessage.success('删除成功!')
          this.load()
        }
    ...
    updateDevices(){
      //修改
      request.put("/mois/update",this.form).then(res =>{
        if (res.code==0){
